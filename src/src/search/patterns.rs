...
impl InternalSboxPattern {
    ...
    fn extend(
        ...
        if !self.is_complete() {
            ...
            let trivial = f64::from(
                *property_values[self.determined_length]
                    .first()
        ...
        if let Some(x) = corr_idx {
            if x + 1 < property_values[self.determined_length - 1].len() {
                ...
                let trivial = f64::from(
                    *property_values[self.determined_length - 1]
                        .first()
                        .expect("No values found."),
                );
                    ...
                    PropertyType::Linear => {
                        new_pattern.value /=
                            (f64::from(property_values[self.determined_length - 1][x]) / trivial)
                                .powi(2);
                        new_pattern.value *=
                            (f64::from(property_values[self.determined_length - 1][x + 1])
                                / trivial)
                                .powi(2);
                    }
...
impl SboxPattern {
    ...
    fn new(
        ...
        let pattern: Vec<_> = internal_sbox_pattern
            .pattern
            .iter()
            .map(|x| x.expect("Tried to convert incomplete pattern."))
            .enumerate()
            .filter(|&(_, x)| x != non_property)
            .map(|(i, x)| (cipher.sbox_pos_in(i), cipher.sbox_pos_out(i), i, x))
            .collect();
        ...
        for i in 0..self.counter.len() {
            ...
            let app = match property_filter {
                ...
                PropertyFilter::Output => {
                    value_maps[sbox].get_output(val).unwrap()[self.counter[i]]
                }
            };
    ...
    pub fn num_prop(&self, value_maps: &[ValueMap]) -> usize {
        self.pattern
            .iter()
            .fold(1, |acc, &(_, _, j, x)| acc * value_maps[j].len_of(x))
    }
    ...
    pub fn num_input(&self, value_maps: &[ValueMap]) -> usize {
        self.pattern
            .iter()
            .fold(1, |acc, &(_, _, j, x)| acc * value_maps[j].len_of_input(x))
    }
    ...
    pub fn num_output(&self, value_maps: &[ValueMap]) -> usize {
        self.pattern
            .iter()
            .fold(1, |acc, &(_, _, j, x)| acc * value_maps[j].len_of_output(x))
    }
}

/// Creates a vector of S-box patterns, sorted by their values.
/// Also returns the associated `ValueMap`.
pub fn get_sorted_patterns(
    cipher: &dyn Cipher,
    pattern_limit: usize,
    property_type: PropertyType,
) -> (Vec<SboxPattern>, Vec<ValueMap>) {
    // Generate property map and get S-box property values
    let value_maps: Vec<_> = (0..cipher.num_sboxes())
        .map(|i| ValueMap::new(cipher.sbox(i), property_type))
        .collect();

    let mut property_values: Vec<Vec<_>> = value_maps
        .iter()
    ...
    while sbox_patterns.len() < pattern_limit {
        ...
        if heap.is_empty() {
            let sbox_patterns: Vec<SboxPattern> = sbox_patterns
                .iter()
