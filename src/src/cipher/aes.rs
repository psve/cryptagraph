...
impl Cipher for Aes {
    ...
    */
    fn linear_layer(&self, input: u128) -> u128{
        ...
        for i in 0..4 {
            ...
            y ^= (((x >> (0+32*i)) & 0xff) 
              ^ aes_times2(((x >> (0+32*i)) & 0xff)^((x >> (8+32*i)) & 0xff)) 
              ^ t) << (0+32*i);
            y ^= (((x >> (8+32*i)) & 0xff) 
    ...
    */
    fn linear_layer_inv(&self, input: u128) -> u128 {
        ...
        for i in 0..4 {
            let u = aes_times2(aes_times2(((x >> (0+32*i)) & 0xff) ^ ((x >> (16+32*i)) & 0xff)));
    
            y ^= (((x >> (0+32*i)) & 0xff) ^ u) << (0+32*i);
    ...
    */
    fn key_schedule(&self, rounds : usize, key: &[u8]) -> Vec<u128> {
        ...
        for _ in 0..rounds {
            ...
            for j in 0..4 {
                k ^= (self.sbox.table[((tmp >> (8*j)) & 0xff) as usize] as u128) << (8*j);
            }
    ...
    */
    fn encrypt(&self, input: u128, round_keys: &Vec<u128>) -> u128 {
        ...
        for i in 0..9 {
            ...
            for j in 0..16 {
                tmp ^= (self.sbox.table[((output >> (j*8)) & 0xff) as usize] as u128) << (j*8);
            }
    ...
    */
    fn decrypt(&self, input: u128, round_keys: &Vec<u128>) -> u128 {
        ...
        for j in 0..16 {
            output ^= (self.isbox.table[((tmp >> (j*8)) & 0xff) as usize] as u128) << (j*8);
        }
