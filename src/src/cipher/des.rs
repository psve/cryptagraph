...
impl Des {
    ...
    pub fn new() -> Des {
        ...
        let permute = [
            7, 28, 21, 10, 26, 2, 19, 13, 23, 29, 5, 0, 18, 8, 24, 30, 22, 1, 14, 27, 6, 9, 17, 31,
...
impl Cipher for Des {
    ...
    fn sbox_mask_transform(
        ...
        match property_type {
            PropertyType::Linear => {
                let input =
                    self.inv_expand(input & 0xffffffffffff) ^ (self.inv_expand(input >> 48) << 32);
                let output = self.linear_layer(output & 0xffffffff)
                    ^ ((self.linear_layer(output >> 32)) << 32);

                let mut alpha = ((output & 0xffffffff) << 32) ^ (output >> 32);
                alpha ^= input & 0xffffffff;

                let mut beta = ((output & 0xffffffff) << 32) ^ (output >> 32);
