...
impl Cipher for Skinny128 {
    ...
    */
    fn encrypt(&self, input: u128, round_keys: &[u128]) -> u128 {
        ...
        for (i, round_key) in round_keys.iter().enumerate().take(40) {
            // Apply S-box
            let mut tmp = 0;

            for j in 0..16 {
                tmp ^= u128::from(self.sbox.table[((output >> (8*j)) & 0xff) as usize]) << (8*j);
            }
    ...
    */
    fn decrypt(&self, input: u128, round_keys: &[u128]) -> u128 {
        ...
        for i in 0..40 {
            ...
            for j in 0..16 {
                tmp ^= u128::from(self.isbox.table[((output >> (8*j)) & 0xff) as usize]) << (8*j);
            }
